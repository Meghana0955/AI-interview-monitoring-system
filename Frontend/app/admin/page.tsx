...
const ALERT_BREAKDOWN = [
  { label: "Eye Deviation",    pct: 42, color: "#7c3aed" },
  { label: "Background Noise", pct: 31, color: "#8b5cf6" },
  { label: "Face Absence",     pct: 18, color: "#a78bfa" },
  ...
  return (
    ...
    <div className="min-h-screen bg-[#08080f] pt-24">
      <div className="max-w-7xl mx-auto px-4 md:px-6 py-6">
        ...
        >
          <StatCard label="Total Candidates" value={47} sub="This session" icon={<Users className="w-4 h-4" />} accentColor="#7c3aed" />
        ...
        <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }} transition={{ delay:0.15 }} className="mb-4">
          <Card>
            <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
              <h3 className="text-[11px] font-semibold text-white/35 uppercase tracking-[0.18em]">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="pl-9 pr-3 py-1.5 bg-[#0d0d14] border border-white/[0.07] rounded-xl text-xs text-white outline-none focus:border-[#7c3aed]/60 focus:ring-2 focus:ring-[#7c3aed]/15 transition-all duration-200 w-48"
                />
              </div>
            </div>
            <div className="overflow-x-auto">
                ...
                <thead>
                  <tr className="border-b border-white/[0.06]">
                    {["Candidate","Dept","Eye","Face","Voice","Risk Score","Level","Action"].map(h => (
                      <th key={h} className="text-left text-[10px] font-semibold text-white/25 uppercase tracking-[0.15em] px-3 py-2.5 whitespace-nowrap">
                        {h}
                      </th>
                    ))}
                  </tr>
                  ...
                  {filtered.map((c) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-3 py-3 font-mono text-[#a78bfa] text-sm">{c.eye}</td>
                        <td className="px-3 py-3 font-mono text-[#8b5cf6] text-sm">{c.face}</td>
                        <td className="px-3 py-3 font-mono text-[#7c3aed] text-sm">{c.voice}</td>
                        <td className="px-3 py-3">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="font-mono text-xs text-white/40">{c.risk}</span>
                          </div>
                        </td>
                        <td className="px-3 py-3">
              ...
              {filtered.length === 0 && (
                <div className="text-center py-10 text-white/25 text-sm">No candidates match your search.</div>
              )}
            </div>
          </Card>
        </motion.div>
